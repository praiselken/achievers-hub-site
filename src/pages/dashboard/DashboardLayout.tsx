...
const TABS: { key: Tab; label: string; path: string; icon: React.ReactNode }[] = [
  ...
  {
    key: 'topics', label: 'Topics', path: '/dashboard/topics',
  ...
  {
    key: 'spec', label: 'Spec Map', path: '/dashboard/spec',
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-1">
          {TABS.map(t => (
            <Link key={t.key} to={t.path}
              ...
              {t.label}
              {t.key === 'daily5' && <span className="ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: 'var(--purple)' }}>Today</span>}
            </Link>
          ))}
        </nav>
