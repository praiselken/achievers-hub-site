  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.4)' }}>
      <div className="bg-white rounded-3xl p-7 w-full max-w-md" style={{ boxShadow: '0 24px 64px rgba(28,28,46,0.18)' }}>
        <h3 className="font-display font-bold text-xl text-[var(--color-ink-900)] mb-1">Log your score</h3>
        <p className="text-sm text-[var(--color-ink-500)] mb-6">{paper.title}</p>
        <div className="flex gap-3 mb-4">
          <div className="flex-1">
            <label className="text-xs font-semibold text-[var(--color-ink-500)] mb-1 block">Your score</label>
          ...
          <div className="flex-1">
            <label className="text-xs font-semibold text-[var(--color-ink-500)] mb-1 block">Out of</label>
        ...
        {score && max && (
          <div className="rounded-xl px-4 py-3 mb-4 text-center"
            ...
            <span className="font-display font-bold text-2xl" style={{ color: 'var(--color-primary-700)' }}>
              {Math.round((parseInt(score) / parseInt(max)) * 100)}%
            </span>
          </div>
        )}
        <div className="mb-5">
          <label className="text-xs font-semibold text-[var(--color-ink-500)] mb-1 block">Notes (optional)</label>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-slate-200 font-semibold text-sm text-[var(--color-ink-500)]">
            Cancel
          </button>
          <button
...
function PaperCard({ paper, log, onLog }: { paper: Paper; log?: PaperLog; onLog: () => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-200" style={{ boxShadow: 'var(--shadow-soft)' }}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-[var(--color-ink-300)]">{paper.year}</span>
            <span className="text-[var(--color-ink-300)]">·</span>
            <span className="text-xs text-[var(--color-ink-300)]">Paper {paper.paper_number}</span>
          </div>
        </div>
        {pct !== null && (
          <div className="text-right flex-shrink-0">
            <div className="font-display font-bold text-xl" style={{ color: scoreColor }}>{pct}%</div>
            <div className="text-[10px] text-[var(--color-ink-300)]">{log!.score}/{log!.max_score}</div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {paper.pdf_url ? (
          <a href={paper.pdf_url} target="_blank" rel="noopener noreferrer"
             className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg no-underline transition-all"
          ...
          <span className="text-xs text-[var(--color-ink-300)] px-3 py-1.5 rounded-lg border border-dashed border-slate-200">
            PDF coming soon
          </span>
        )}
        ...
        {paper.examiner_url && (
          <a href={paper.examiner_url} target="_blank" rel="noopener noreferrer"
             className="text-xs font-semibold px-3 py-1.5 rounded-lg no-underline"
             style={{ background: 'var(--feature-daily-bg)', color: 'var(--feature-daily-strong)' }}>
            Examiner report
          </a>
        )}
        <button onClick={onLog}
          className="text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all"
          style={{ borderColor: pct !== null ? 'var(--color-primary-200)' : 'var(--color-border)', color: pct !== null ? 'var(--color-primary-500)' : 'var(--color-ink-500)' }}>
...
export default function PastPapersTab() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="font-display font-bold text-2xl text-[var(--color-ink-900)]">Past Papers</h1>
        <p className="text-sm text-[var(--color-ink-500)] mt-0.5">Download, attempt, then log your score to track improvement.</p>
      </div>
      ...
      {logsCount > 0 && (
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl p-4 border border-slate-200">
            <div className="text-xs font-semibold text-[var(--color-ink-300)] uppercase tracking-wider mb-1">Papers logged</div>
            <div className="font-display font-bold text-3xl text-[var(--color-ink-900)]">{logsCount}</div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-slate-200">
            <div className="text-xs font-semibold text-[var(--color-ink-300)] uppercase tracking-wider mb-1">Average score</div>
            <div className="font-display font-bold text-3xl" style={{ color: avgPct && avgPct >= 70 ? 'var(--color-success-600)' : 'var(--color-primary-700)' }}>
              {avgPct !== null ? `${avgPct}%` : '—'}
            </div>
          </div>
        </div>
      ...
      {/* Papers */}
      {loading ? (
        <p className="text-sm text-[var(--color-ink-300)] text-center py-12">Loading papers…</p>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16">
          <div className="text-4xl mb-3">📄</div>
          <h3 className="font-display font-bold text-[var(--color-ink-900)] mb-2">Papers coming soon</h3>
          <p className="text-sm text-[var(--color-ink-500)] max-w-sm mx-auto">
