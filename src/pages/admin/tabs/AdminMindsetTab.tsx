...
import { DEMO_ADMIN_MINDSET } from '../../../lib/demoData';
...
export default function AdminMindsetTab() {
  ...
  async function load() {
    if (isDemoMode()) { setPrompts(DEMO_ADMIN_MINDSET as never); setLoading(false); return; }
    if (!supabase) return;
  ...
  async function deletePrompt(id: string) {
    if (isDemoMode()) {
      setPrompts((prev) => prev.filter((row) => row.id !== id));
      setDeleteId(null);
      return;
    }
    if (!supabase) return;
    ...
    <div className="flex flex-col gap-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {filtered.length === 0 ? (
            <p className="text-sm text-center py-10" style={{ color: 'rgba(255,255,255,0.25)' }}>No prompts for this month</p>
          ) : filtered.map(p => (
            <div key={p.id} className="rounded-2xl p-5 flex flex-col gap-2"
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0"
                       style={{ background: 'rgba(169,125,192,0.2)', color: 'var(--color-primary-200)' }}>
                    {p.day}
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-white mt-0.5 leading-snug">"{p.confession}"</p>
                  </div>
                </div>
              ...
              {p.reflection && (
                <p className="text-xs leading-relaxed ml-13 pl-1" style={{ color: 'rgba(255,255,255,0.4)', marginLeft: '52px' }}>
                  {p.reflection}
                </p>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-40 flex items-start justify-end p-4" style={{ background: 'rgba(0,0,0,0.6)' }}
          ...
          <div className="h-full max-h-screen overflow-y-auto rounded-2xl w-full max-w-lg flex flex-col gap-4 p-6"
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={save} disabled={saving || !form.confession} className="flex-1 py-3 rounded-xl font-bold text-sm text-white" style={{ background: 'linear-gradient(135deg, var(--color-primary-300), var(--color-primary-400))', opacity: saving ? 0.7 : 1 }}>
                {saving ? 'Saving…' : editing ? 'Save changes' : 'Add prompt'}
              </button>
            </div>
          </div>
          <style>{`.mind-inp { width:100%; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 10px 14px; font-family: inherit; font-size: 14px; color: white; outline: none; } .mind-inp::placeholder { color: rgba(255,255,255,0.25); } .mind-inp:focus { border-color: rgba(169,125,192,0.5); } .mind-inp option { background: #241041; }`}</style>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
          <div className="rounded-2xl p-6 max-w-sm w-full" style={{ background: '#241041', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => deletePrompt(deleteId)} className="flex-1 py-2.5 rounded-xl font-bold text-sm" style={{ background: 'var(--color-accent-600)', color: '#fff' }}>Delete</button>
            </div>
          </div>
        </div>
      )}
